...
const JoinAsInstructor = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (otpVerified === true) {
      ...
      try {
        const response = await axios.post(`${backenduri}/instructor/register`, formData, {
          headers: {
        ...
        if (response.status === 200) {
          const { token, id } = response.data; // Extract token and id
          if (!token || !id) {
            throw new Error('Missing token or id in the response');
          }
          
          login(token, 'instructor', id); // Automatically log in the user
          toast.success('Details submitted successfully! Redirecting...');
  ...
  return (
    <div className="full-page">
      <AuthHeader />
      <h4 className="subtitle">Ready to Grow? Create an account and start teaching!</h4>
      <div className="instructor-container">
        <form onSubmit={handleSubmit}>
          ...
          <div className="row-check">
            ...
            <p>
              I certify that the information I have provided is accurate and complete. I understand that any false or misleading details may lead to the rejection of my application or future disqualification.
            </p>
          </div>
